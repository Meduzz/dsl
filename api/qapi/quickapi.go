...
func Quickapi(api *api.Api, prefix, name string, entity any) {
	create := api.POST(fmt.Sprintf("/%s/%s/", prefix, name))
	...
	read := api.GET(fmt.Sprintf("/%s/%s/:id", prefix, name))
	...
	update := api.PUT(fmt.Sprintf("/%s/%s/:id", prefix, name))
	...
	remove := api.DELETE(fmt.Sprintf("/%s/%s/:id", prefix, name))
	remove.Description = fmt.Sprintf("Delete an %s entity by id", name)
	remove.PathVariable("id")

	search := api.GET(fmt.Sprintf("/%s/%s/", prefix, name))
	...
	patch := api.PATCH(fmt.Sprintf("/%s/%s/:id", prefix, name))
